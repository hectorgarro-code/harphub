...
/**
 * Engine de Minijuegos para Ukelele.
...
export const useGameEngine = ({ tuning, fretCount, addPoints, unlockAchievement, useFlats = false }) => {
    ...
    const [stats, setStats]                       = useState(() => {
        const saved = localStorage.getItem('ukelele_master_stats');
    ...
    useEffect(() => {
        if (gameState === 'playing' && gameTimer > 0) {
            ...
            const sessions = parseInt(localStorage.getItem('ukelele_master_sessions') || '0') + 1;
            localStorage.setItem('ukelele_master_sessions', sessions.toString());
            if (sessions >= 10 && unlockAchievement) unlockAchievement('ukelele_master');
        }
    }, [gameState, gameTimer]);
    ...
    const handleGameAction = (input) => {
        ...
        if (gameMode === 'identifier') {
            if (input === gameChallenge.note) {
                ...
                setStats(newStats);
                localStorage.setItem('ukelele_master_stats', JSON.stringify(newStats));
